"""Cache Invalidator — middleware for event payloads."""
...
class CacheMiddleware:
    """Middleware for Cache Invalidator event payloads."""
...
def parse_events(payload: str) -> List[Dict[str, Any]]:
    ...
    return [CacheMiddleware._coerce(item) for item in raw]


def flush_event_to_str(
    record: Dict[str, Any], indent: Optional[int] = None
) -> str:
    """Convenience wrapper — serialise a Event to a JSON string."""
    if indent is None:
        return CacheMiddleware.dumps(record)
